...
private:
    ...
    vector<Node> nodes;
    void showTree();
    pair<int, char> traverse_on_alphabet(int, double);
    int traverse_on_wavelet(int, char);
    bool isLeaf(int);
    bool isEmptyNode(int);
    double idx2character(int);
public:
    ~WaveletTree(){};
...
pair<int, char> WaveletTree::traverse_on_alphabet(int n_idx, double v){
    if(n_idx >= nodes.size()){
...
double WaveletTree::idx2character(int n_idx){
...
WaveletTree::WaveletTree(vector<double>& _S){//{{{
    n = _S.size();
    if( n > UB_TEXT_SIZE ){
        fprintf(stderr,
    ...
    vector<double> temp;
    copy(_S.begin(), _S.end(), back_inserter(temp));
    ...
    dict.resize(sigma, -1.);
    vector<double>::iterator it_b = temp.begin(), end_it_b = temp.end();
    for(int d_idx=0; it_b != end_it_b; ++it_b){
        dict[d_idx++] = *it_b;
    }
    //printf("a dictionary is constructed.\n");
    ...
    for(int i=sigma-1; i>0; i--){
        ...
        double l_child = nodes[2*i].max,
               r_child = nodes[2*i+1].max;
...
double WaveletTree::access(int i){//{{{
    int c=0;
    char b;
    for(
